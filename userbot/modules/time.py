...
async def time_func(tdata):
    ...
    if c_name != COUNTRY:
        await tdata.edit(f"`Sekarang`  **{dtnow}**  `di {c_name}({time_zone} timezone).`")
        return

    elif COUNTRY:
        await tdata.edit(
            f"`Sekarang`  **{dtnow}**  `disini, di {COUNTRY}" f"({time_zone} timezone).`"
        )
        return
...
async def date_func(dat):
    ...
    if c_name != COUNTRY:
        await dat.edit(f"`Sekarang`  **{dtnow}**  `di {c_name}({time_zone} timezone).`")
        return
