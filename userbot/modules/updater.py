    ...
    if HEROKU_API_KEY is not None:
        ...
        if BOTLOG:
            await event.client.send_message(
                BOTLOG_CHATID, "#UPDATE \n" "UserBot FeRuBoT Anda sudah ter-update, yaaay"
            )
...
async def upstream(event):
    ...
    try:
        ...
        if conf is None:
            return await event.edit(
                f"`Sayangnya, direktori {error} tampaknya bukan repositori git Tapi kita."
                "\nbisa memperbaikinya dengan memperbarui paksa userbot menggunakan .update sekarang.`"
            )
        repo = Repo.init()
    ...
    if ac_br != UPSTREAM_REPO_BRANCH:
        await event.edit(
            ...
            "silakan checkout ke branch resmi mana pun`"
        )
        return repo.__del__()
    try:
    ...
    if force_update:
        ...
        await event.edit("`Sedang update FeRuBoT, Harap Tunggu....`")
    if conf == "now":
