...
async def who(event):
    ...
    await event.edit(
        "`Harap tunggu sementara saya mencuri beberapa data dari *Global Network Zone*...`"
    )
    ...
    if replied_user is None:
        await event.edit("`Ini adalah admin anonim di grup ini.\nTidak dapat mengambil info`")
        return

    try:
        photo, caption = await fetch_info(replied_user, event)
    except AttributeError:
        await event.edit("`Tidak dapat mengambil info pengguna itu.`")
        return
...
async def fetch_info(replied_user, event):
    ...
    caption += f"NAMA AWAL     : {first_name}\n"
...
CMD_HELP.update(
    {
        "whois": ".whois <username> atau balas teks seseorang dengan .whois\
        \nPenggunaan: Mendapat info pengguna."
    }
)
