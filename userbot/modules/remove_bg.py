...
async def kbg(remob):
    ...
    if REM_BG_API_KEY is None:
        await remob.edit(
            "`Error: API Key REMOVE BG hilang! Tambahkan ke environment vars atau config.env.`"
        )
        return
    ...
    if "image" in contentType:
        ...
        await remob.edit(
            "**Kesalahan (Sepertinya API Key tidak valid?)**\n`{}`".format(
...
CMD_HELP.update(
    {
        "rbg": ".rbg <Link gambar> atau balas gambar yg ingin dihapus latar belakang (Warning: tidak bekerja pada stiker!.)\
