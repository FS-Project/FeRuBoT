...
async def fban(event):
    ...
    if failed:
        status = f"Gagal melakukan fban di {len(failed)}/{total} feds.\n"
        for i in failed:
            status += "• " + i + "\n"
    else:
        status = f"Suksess! Fbanned di {total} feds."
...
async def unfban(event):
    ...
    if failed:
        status = f"Gagal membatalkan fban di {len(failed)}/{total} feds.\n"
        for i in failed:
            status += "• " + i + "\n"
    else:
        status = f"Suksess! Un-fbanned di {total} feds."
...
async def listf(event):
    ...
    msg = "**Federations yang terhubung:**\n\n"
...
CMD_HELP.update(
    {
        "fban": ".fban <id/username> <alasan>"
        "\nPenggunaan: Larang pengguna dari federasi yang terhubung."
        "\nAnda dapat membalas ke pengguna yang ingin Anda fban atau secara manual memasukkan username/id."
        "\n\n.unfban <id/username> <alasan>"
        "\nPenggunaan: Sama seperti fban tetapi unbans pengguna"
        "\n\n.addf <nama>"
        "\nPenggunaan: Menambahkan grup saat ini dan menyimpannya sebagai <nama> di federasi terhubung."
