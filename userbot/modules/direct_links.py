...
async def direct_link_generator(request):
    ...
    if not links:
        reply = "`Tidak ada link yang ditemukan!`"
...
async def zippy_share(url: str) -> str:
    ...
    try:
        ...
        reply = "`Tidak ditemukan tautan ZippyShare`\n"
        return reply
...
async def yandex_disk(url: str) -> str:
    ...
    try:
        ...
        reply = "`Tidak ditemukan tautan Yandex.Disk`\n"
        return reply
    ...
    try:
        ...
        reply += "`Error: File tidak ditemukan / Batas unduhan tercapai`\n"
        return reply
    return reply
...
async def cm_ru(url: str) -> str:
    ...
    try:
        ...
        reply = "`Tidak ditemukan tautan cloud.mail.ru`\n"
        return reply
    ...
    try:
        ...
        reply += "`Error: Tidak dapat mengekstrak tautan`\n"
        return reply
    except IndexError:
        return reply
...
async def mediafire(url: str) -> str:
    ...
    try:
        ...
        reply = "`Tidak ditemukan tautan MediaFire`\n"
        return reply
    reply = ""
...
async def sourceforge(url: str) -> str:
    ...
    try:
        ...
        reply = "`Tidak ditemukan tautan SourceForge`\n"
        return reply
...
async def osdn(url: str) -> str:
    ...
    try:
        ...
        reply = "`Tidak ditemukan tautan OSDN`\n"
        return reply
...
async def github(url: str) -> str:
    ...
    try:
        ...
        reply = "`Tidak ditemukan tautan rilis GitHub`\n"
        return reply
    reply = ""
    dl_url = ""
...
async def androidfilehost(url: str) -> str:
    ...
    try:
        ...
        reply = "`Tidak ditemukan tautan AFH`\n"
        return reply
    ...
    mirrors = None
    reply = ""
    error = "`Error: Tidak dapat menemukan Mirror untuk link`\n"
    try:
        req = session.post(
...
async def uptobox(request, url: str) -> str:
    ...
    try:
        ...
        await request.edit("`Tidak ditemukan tautan uptobox.`")
        return
    if USR_TOKEN is None:
        await request.edit("`Set USR_TOKEN_UPTOBOX terlebih dahulu!`")
        return
    if link.endswith("/"):
        index = -2
    else:
...
CMD_HELP.update(
    {
        "direct": ".direct <url>\n"
        "Penggunaan: Balas tautan atau tempel URL untuk\n"
