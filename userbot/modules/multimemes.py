...
async def glitch(event):
    if not event.reply_to_msg_id:
        await event.edit("`Saya Tidak Akan Menyesatkan Hantu!`")
        return
    ...
    await event.edit("`Mengupload Media...`")
    c_time = time.time()
...
async def mim(event):
    if not event.reply_to_msg_id:
        await event.edit(
            "`Syntax: membalas gambar dengan .mmf` 'teks di atas' ; 'teks di bawah' "
        )
        return
...
CMD_HELP.update(
    {
        "glitch": ".glitch <1-8>\
            \nPenggunaan: Balas stiker/gambar dan kirim dengan .\
            \nvalue range 1-8 jika tidak akan menggunakan default=2"
    }
)
...
CMD_HELP.update(
    {
        "memify": ".mmf texttop ; textbottom\
            \nPenggunaan: Balas stiker/gambar/gif dan kirim dengan cmd."
    }
)
...
CMD_HELP.update(
    {
        "quotly": ".q \
          \nPenggunaan: Tingkatkan teks Anda ke stiker."
    }
)
...
CMD_HELP.update(
    {
        ...
        "\nPenggunaan: Membalas gambar / stiker yang sesuai!"
        "\n@hazmat_suit_bot"
    }
)
...
CMD_HELP.update(
    {
        ...
        "\nPenggunaan: gambar / stiker deepfry dari balasan."
        "\n@image_deepfrybot"
    }
)
...
CMD_HELP.update(
    {
        "sangmata": ".sg \
          \nPenggunaan: Curi nama Anda atau teman."
    }
)
...
CMD_HELP.update(
    {
        "waifu": ".waifu \
          \nPenggunaan: Tingkatkan teks Anda dengan template gadis anime cantik. \
          \n@StickerizerBot"
    }
)
