...
async def anime(event):
    ...
    if query:
        ...
        await event.edit("`Bruh.. Apa yang harus saya cari ?`")
    ...
    try:
        ...
        await event.edit("`Terjadi kesalahan saat menghubungkan ke API. Silakan coba lagi!`")
        return
    if res:
    ...
    rep += f'<b>Baca lebih lanjut:</b> <a href="{url}">MyAnimeList</a>'
...
async def manga(event):
    ...
    if not query:
        await event.edit("`Bruh.. Beri aku Sesuatu untuk Ditelusuri`")
        return
    res = ""
    manga = ""
    try:
        ...
        await event.edit("`Terjadi kesalahan saat menghubungkan ke API. Silahkan coba lagi!`")
        return ""
    if res:
        try:
...
async def site_search(event):
    ...
    if search_query:
        ...
        await event.edit("`Uuf Bro.. Berikan sesuatu untuk dicari`")
        return
    ...
    if site == "kaizoku":
        ...
        if search_result:
            result = f"<a href='{search_url}'>Klik Di Sini Untuk Hasil Lebih Lanjut</a> <b>of</b> <code>{html.escape(search_query)}</code> <b>on</b> <code>AnimeKaizoku</code>: \n\n"
            ...
            result = f"<b>Tidak ditemukan hasil dari</b> <code>{html.escape(search_query)}</code> <b>on</b> <code>AnimeKaizoku</code>"
            await event.edit(result, parse_mode="HTML")
        ...
        result = f"<a href='{search_url}'>Klik Di Sini Untuk Hasil Lebih Lanjut</a> <b>of</b> <code>{html.escape(search_query)}</code> <b>on</b> <code>AnimeKayo</code>: \n\n"
        for entry in search_result:
...
async def character(event):
    ...
    caption += f" [Baca lebih lanjut]({mal_url})..."
    await event.delete()
...
async def get_anime(message):
    try:
        ...
        if message.reply_to_msg_id:
            ...
            await message.reply(
                "Anda tidak memberikan apa pun untuk dicari. (｡ì _ í｡)\n `Gunakan: .scanime <anime name>`"
            )
            return
    except Exception as err:
        await message.edit(f"**Menemukan Pengecualian Tidak Diketahui**: \n{err}")
        return
    ...
    try:
        ...
        await p_rm.edit(f"Hasil Tidak Ditemukan untuk {query}")
        return
    except Exception as err:
        await p_rm.edit(f"**Menemukan Pengecualian Tidak Diketahui**: \n{err}")
        return
...
[📚 Info lebih lanjut]({mal_dir_link})
"""
...
async def whatanime(e):
    ...
    if not media:
        await e.edit("`Media dibutuhkan`")
        return
    ig = is_gif(media) or is_video(media)
    if not is_image(media) and not ig:
        await e.edit("`Media harus berupa gambar atau gif atau video`")
        return
    filename = "file.jpg"
    if not ig and isinstance(media, MessageMediaDocument):
        ...
        for i in attribs:
            if isinstance(i, DocumentAttributeFilename):
                ...
                break
    await e.edit("`Mendownload gambar..`")
...
CMD_HELP.update(
    {
        "anime": ".anime <anime>\
    \nKegunaan: Kembali dengan informasi Anime.\
    \n\n.manga <manga name>\
    \nKegunaan: Kembali dengan informasi Manga.\
    \n\n.akaizoku or .akayo <anime name>\
    \nKegunaan: Kembali dengan tautan Unduh Anime.\
    \n\n.char <character name>\
    \nKegunaan: Kembali dengan informasi karakter.\
    \n\n.upcoming\
    \nKegunaan: Kembali dengan informasi Anime Mendatang.\
    \n\n.scanime <anime> or .sanime <anime>\
    \nKegunaan: Mencari Anime.\
    \n\n.smanga <manga>\
    \nKegunaan: Mencari manga.\
