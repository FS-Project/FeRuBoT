...
async def rem_locks(event):
    ...
    if input_str == "msg":
        ...
        if not input_str:
            await event.edit("`Saya tidak bisa membuka kunci apa pun!!`")
            return
        else:
            await event.edit(f"`Jenis buka kunci tidak valid:` {input_str}")
            return
    ...
    try:
        ...
        await event.edit(f"`Membuka {what} untuk obrolan ini !!`")
    except BaseException as e:
        await event.edit(f"`Apakah saya memiliki hak yang tepat untuk itu??`\n**Error:** {str(e)}")
        return
...
CMD_HELP.update(
    {
        "locks": ".lock <all (or) type(s)> atau .unlock <all (or) type(s)>\
\nPenggunaan: Memungkinkan Anda untuk mengunci/membuka kunci beberapa jenis pesan umum dalam obrolan.\
[NOTE: Membutuhkan hak admin yang tepat dalam obrolan !!]\
\n\nJenis pesan yang tersedia untuk mengunci/membuka kunci adalah: \
