...
async def device_info(request):
    """ dapatkan info dasar perangkat android dari codename """
    ...
    if codename:
        ...
        await request.edit("`Gunakan: .device <codename> / <model>`")
        return
    data = json.loads(
        get(
    ...
    if results:
        reply = f"**Hasil Pencarian Untuk {codename}**:\n\n"
        for item in results:
            reply += (
                f"**Brand**: {item['brand']}\n"
                f"**Name**: {item['name']}\n"
        ...
        reply = f"`Tidak dapat menemukan info tentang {codename}!`\n"
    await request.edit(reply)


@register(outgoing=True, pattern=r"^.codename(?: |)([\S]*)(?: |)([\s\S]*)")
async def codename_info(request):
    """ cari nama codename android """
    textx = await request.get_reply_message()
    ...
    if brand and device:
        ...
        await request.edit("`Gunakan: .codename <brand> <device>`")
        return
    ...
    if results:
        reply = f"**Hasil pencarian {brand} {device}**:\n\n"
        if len(results) > 8:
            results = results[:8]
        for item in results:
            reply += (
...
async def download_api(dl):
    ...
    if md5_origin is None and i is None:
        await dl.edit("`Tidak ada versi kecocokan yang tersedia...`")
    if URL.endswith("/"):
    ...
    download.click()
    await dl.edit("`Download dimulai...`")
    ...
    while complete is False:
        ...
        if downloaded == file_size:
            ...
            if md5_origin == MD5:
                ...
                await dl.edit("`Download rusak...`")
                os.remove(file_path)
                driver.quit()
                return
    await dl.respond(f"`{file_name}`\n\n" f"Berhasil di download ke `{file_path}`.")
    await dl.delete()
    driver.quit()
    return
...
async def devices_specifications(request):
    ...
    if brand and device:
        ...
        await request.edit("`Gunakan: .specs <brand> <device>`")
        return
    all_brands = (
        BeautifulSoup(
    ...
    try:
        ...
        await request.edit(f"`{brand} adalah merek yang tidak dikenal!`")
...
async def twrp(request):
    """ dapatkan perangkat android twrp """
    textx = await request.get_reply_message()
    ...
    if device:
        ...
        await request.edit("`Gunakan: .twrp <codename>`")
        return
...
\nKegunaan: Cari nama codename perangkat android.\
\n\n.pixeldl **<download.pixelexperience.org>**\
\nKegunaan: Unduh ROM pixel exprience ke server userbot Anda.\
\n\n.specs <brand> <device>\
